...
from app.models.models import (
    WorkItem, User, Notification, NotificationStatus
)
...
def _config_value(db: Session, key: str, fallback: str) -> str:
    return str(load_org_config(db).get(key) or fallback)


def _base_url(db: Session) -> str:
    return _config_value(db, "app_url", "http://localhost:8000")


def _item_url(db: Session, item_id: int) -> str:
    return f"{_base_url(db)}/#/items/{item_id}"


def _html_wrapper(db: Session, title: str, body: str) -> str:
    config = load_org_config(db)
    product_name = str(config.get("product_name") or "FinAgent")
    org_name = str(config.get("org_name") or "האגף הפיננסי")
...
{body}
<div class="footer">הודעה זו נשלחה אוטומטית על ידי {product_name} | {org_name}</div>
</div></body></html>"""
